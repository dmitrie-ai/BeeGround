...
namespace Unity.RecordedPlayback.Editor
{
	...
	internal class HubWindow : EditorWindow
	{
		...
		private void Init()
		{
			...
			if (currentSubWindow == null && EditorApplication.isPlayingOrWillChangePlaymode && !string.IsNullOrEmpty(State.currentWindowFocused) && State.currentWindowFocused != GetType().ToString())
			{ 
		...
		void RenderMainView()
		{
			...
			var visualTree = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(HUB_RESOURCES_PATH + $"{WINDOW_FILE_NAME}.uxml");
			baseRoot = rootVisualElement;
			visualTree.CloneTree(baseRoot);

			baseRoot.styleSheets.Add(AssetDatabase.LoadAssetAtPath<StyleSheet>(HUB_RESOURCES_PATH + $"{WINDOW_FILE_NAME}.uss"));
			baseRoot.styleSheets.Add(AssetDatabase.LoadAssetAtPath<StyleSheet>(HUB_RESOURCES_PATH + $"{ALL_WINDOWS_USS_FILE_NAME}"));
			...
			*/
			...
			SetClickAction(ButtonNames.CloudTestingButton, () =>
			{
				SetSubWindow<CloudTestingSubWindow>();
			});
			...
			SetClickAction(ButtonNames.ToolsButton, () =>
			{
				SetSubWindow<ToolsSubWindow>();
			});
			...
			SetClickAction(ButtonNames.SettingsButton, () =>
			{
				SetSubWindow<SettingsSubWindow>();
			});
			
			SetClickAction(ButtonNames.TestGenerationButton, () =>
			{
				SetSubWindow<CodeGenerationSubWindow>();
			});
		...
		private string GetTutorialPopupMessage() 
		{
			...
			message = "Thank you for trying out the Unity Automated QA package. This tool will make testing your software faster, scalable, more efficient, and reliable. Anyone of any technical level can use this package to author automated UI tests. Let's get started by creating a simple recording of you doing just about anything in your application.";
			return message;
		}
		...
		internal Button AddHubBackButton()
		{
			...
			goBackToHubButton.clickable.clicked += () =>
			{
				GoBackToHub();
			};
		...
		{
			List<Button> buttons = root.Q<VisualElement>("PageButtonsContainer").Query<Button>(className: "editor-window-button").ToList();
			...
			root.Q<Label>("TutorialInstructions").text = "First, select the Recorded Playback button highlighted below.";			
		}
